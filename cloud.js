...
async function cloudLoad(isRefresh=false){
  ...
  const taskSourcesChanged=typeof syncTaskSources==='function'&&syncTaskSources({persist:false});cloudReady=true;render();renderParasites();renderTracker();renderBusiness();cloudSetState('ALL CHANGES SAVED',true);toast(isRefresh?'TEAM CHANGES LOADED':'SHARED TRACKER LOADED');if(!businessInitialized||!categoriesInitialized||!tasksInitialized||!optionsInitialized||recoveredMissingBusiness||needsIdeaMigration||taskSourcesChanged)window.skinatorCloudSave();
}
