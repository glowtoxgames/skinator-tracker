...
async function cloudHydrate(kind,source){
  ...
  if(kind==='character')for(const [index,v] of (data.variants||[]).entries()){if(typeof v.gif==='string'&&v.gif.startsWith('storage://')){const path=v.gif.slice(10);cloudAssetPaths.set(`${kind}:${data.id}:variant:${v.id||index}`,path);v.gifStoragePath=path;v.gif=await cloudSignedOptional(path)}}
  return data;
}
