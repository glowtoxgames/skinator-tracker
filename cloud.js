...
async function cloudLoad(isRefresh=false){
  ...
  cloudRemoteKeys.clear();cloudHashes.clear();cloudAssetPaths.clear();const grouped={character:[],modifier:[],npc:[],parasyte:[],achievement:[],outreach:[],publisher:[],idea:[],idea_category:[],ongoing_task:[],task_option:[]};for(const row of rows){const key=`${row.record_type}:${row.id}`;cloudRemoteKeys.add(key);cloudHashes.set(key,JSON.stringify(row.data));grouped[row.record_type]?.push(await cloudHydrate(row.record_type,row.data))}
  ...
  const taskSourcesChanged=typeof syncTaskSources==='function'&&syncTaskSources({persist:false});cloudReady=true;render();renderParasites();renderAchievements();renderTracker();renderBusiness();cloudSetState('ALL CHANGES SAVED',true);toast(isRefresh?'TEAM CHANGES LOADED':'SHARED TRACKER LOADED');if(!businessInitialized||!categoriesInitialized||!tasksInitialized||!optionsInitialized||recoveredMissingBusiness||recoveredMissingAchievements||needsIdeaMigration||taskSourcesChanged)window.skinatorCloudSave();
}
