...
async function cloudLoad(isRefresh=false){
  ...
  const taskSourcesChanged=typeof syncTaskSources==='function'&&syncTaskSources({persist:false});cloudReady=true;render();renderParasites();renderAchievements();renderTracker();renderBusiness();cloudSetReady(true);cloudSetState('ALL CHANGES SAVED',true);toast(isRefresh?'TEAM CHANGES LOADED':'SHARED TRACKER LOADED');if(!businessInitialized||!categoriesInitialized||!tasksInitialized||!optionsInitialized||!influencerAnalysisWasInitialized||influencerAnalysisSeeded||recoveredMissingBusiness||recoveredMissingAchievements||needsIdeaMigration||taskSourcesChanged)window.skinatorCloudSave();
}
