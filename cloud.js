...
function cloudRecords(){return[{kind:'character',items:state.characters},{kind:'modifier',items:state.modifiers},{kind:'npc',items:state.npcs},{kind:'parasyte',items:parasytes},{kind:'outreach',items:outreachRecords},{kind:'publisher',items:publisherRecords},{kind:'idea',items:ideaRecords}]}
...
async function cloudLoad(isRefresh=false){
  ...
  cloudRemoteKeys.clear();cloudHashes.clear();cloudAssetPaths.clear();const grouped={character:[],modifier:[],npc:[],parasyte:[],outreach:[],publisher:[],idea:[]};for(const row of rows){const key=`${row.record_type}:${row.id}`;cloudRemoteKeys.add(key);cloudHashes.set(key,JSON.stringify(row.data));grouped[row.record_type]?.push(await cloudHydrate(row.record_type,row.data))}
  state.characters=grouped.character;state.modifiers=grouped.modifier;state.npcs=grouped.npc;parasytes=grouped.parasyte;
  const {data:plan,error:planError}=await cloudClient.from('skinator_planner').select('data,updated_at').eq('id','main').maybeSingle();if(planError)throw planError;if(plan?.data){Object.keys(planner).forEach(k=>delete planner[k]);Object.assign(planner,plan.data)}const businessInitialized=!!planner.businessInitialized;outreachRecords=businessInitialized?grouped.outreach:(grouped.outreach.length?grouped.outreach:outreachRecords);publisherRecords=businessInitialized?grouped.publisher:(grouped.publisher.length?grouped.publisher:publisherRecords);ideaRecords=businessInitialized?grouped.idea:(grouped.idea.length?grouped.idea:ideaRecords);if(!businessInitialized)planner.businessInitialized=true;localStorage.setItem(BUSINESS_KEY,JSON.stringify({outreach:outreachRecords,publishers:publisherRecords,ideas:ideaRecords}));cloudServerSignature=cloudMakeSignature(rows,plan?.updated_at);
  cloudReady=true;render();renderParasytes();renderTracker();renderBusiness();cloudSetState('ALL CHANGES SAVED',true);toast(isRefresh?'TEAM CHANGES LOADED':'SHARED TRACKER LOADED');if(!businessInitialized)window.skinatorCloudSave();
}
