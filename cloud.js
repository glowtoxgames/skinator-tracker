...
function cloudRecords(){return[{kind:'character',items:state.characters},{kind:'modifier',items:state.modifiers},{kind:'npc',items:state.npcs},{kind:'parasyte',items:parasytes}]}
async function cloudSaveNow(force=false){
  ...
  try{
    ...
    current.forEach(key=>cloudRemoteKeys.add(key));const {error:plannerError}=await cloudClient.from('skinator_planner').upsert({id:'main',data:planner,updated_by:cloudUser.id},{onConflict:'id'});if(plannerError)throw plannerError;cloudSetState('ALL CHANGES SAVED',true);
...
window.skinatorCloudSave=()=>{clearTimeout(cloudTimer);cloudTimer=setTimeout(()=>cloudSaveNow(false),700)};

async function cloudLoad(){
  cloudSetState('LOADING…');const {data:rows,error}=await cloudClient.from('skinator_records').select('id,record_type,data');if(error)throw error;
  if(!rows.length){cloudReady=true;$('cloudMigrate').hidden=false;cloudSetState('READY FOR FIRST UPLOAD');return}
  const grouped={character:[],modifier:[],npc:[],parasyte:[]};for(const row of rows){const key=`${row.record_type}:${row.id}`;cloudRemoteKeys.add(key);cloudHashes.set(key,JSON.stringify(row.data));grouped[row.record_type]?.push(await cloudHydrate(row.record_type,row.data))}
  state.characters=grouped.character;state.modifiers=grouped.modifier;state.npcs=grouped.npc;parasytes=grouped.parasyte;
  const {data:plan,error:planError}=await cloudClient.from('skinator_planner').select('data').eq('id','main').maybeSingle();if(planError)throw planError;if(plan?.data){Object.keys(planner).forEach(k=>delete planner[k]);Object.assign(planner,plan.data)}
  cloudReady=true;render();renderParasytes();renderTracker();cloudSetState('ALL CHANGES SAVED',true);toast('SHARED TRACKER LOADED');
}
async function cloudStart(session){
  ...
  cloudRole=membership.role;$('cloudAuth').hidden=true;$('cloudBar').hidden=false;$('cloudIdentity').textContent=`${membership.display_name||cloudUser.email} // ${cloudRole.toUpperCase()}`;await cloudLoad();
}
