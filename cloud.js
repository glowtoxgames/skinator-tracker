...
async function cloudLoad(isRefresh=false){
  ...
  const {data:plan,error:planError}=await cloudClient.from('skinator_planner').select('data,updated_at').eq('id','main').maybeSingle();if(planError)throw planError;if(plan?.data){Object.keys(planner).forEach(k=>delete planner[k]);Object.assign(planner,plan.data)}const businessInitialized=!!planner.businessInitialized,categoriesInitialized=!!planner.ideaCategoriesInitialized,tasksInitialized=!!planner.ongoingTasksInitialized,optionsInitialized=!!planner.taskOptionsInitialized;outreachRecords=businessInitialized?grouped.outreach:(grouped.outreach.length?grouped.outreach:outreachRecords);publisherRecords=businessInitialized?grouped.publisher:(grouped.publisher.length?grouped.publisher:publisherRecords);ideaRecords=businessInitialized?grouped.idea:(grouped.idea.length?grouped.idea:ideaRecords);ideaCategoryRecords=categoriesInitialized?grouped.idea_category:(grouped.idea_category.length?grouped.idea_category:ideaCategoryRecords);ongoingTaskRecords=tasksInitialized?grouped.ongoing_task:(grouped.ongoing_task.length?grouped.ongoing_task:ongoingTaskRecords);taskOptionRecords=optionsInitialized?grouped.task_option:(grouped.task_option.length?grouped.task_option:taskOptionRecords);if(!businessInitialized)planner.businessInitialized=true;if(!categoriesInitialized)planner.ideaCategoriesInitialized=true;if(!tasksInitialized)planner.ongoingTasksInitialized=true;if(!optionsInitialized)planner.taskOptionsInitialized=true;localStorage.setItem(BUSINESS_KEY,JSON.stringify({outreach:outreachRecords,publishers:publisherRecords,ideas:ideaRecords,ideaCategories:ideaCategoryRecords,ongoingTasks:ongoingTaskRecords,taskOptions:taskOptionRecords}));cloudServerSignature=cloudMakeSignature(rows,plan?.updated_at);
  cloudReady=true;render();renderParasytes();renderTracker();renderBusiness();cloudSetState('ALL CHANGES SAVED',true);toast(isRefresh?'TEAM CHANGES LOADED':'SHARED TRACKER LOADED');if(!businessInitialized||!categoriesInitialized||!tasksInitialized||!optionsInitialized)window.skinatorCloudSave();
}
