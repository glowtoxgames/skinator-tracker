...
async function cloudMapLimit(items,limit,mapper){
  ...
  const workers=Array.from({length:Math.min(limit,items.length)},async()=>{
    while(nextIndex<items.length){const index=nextIndex++;results[index]=await mapper(items[index],index)}
  });
  await Promise.all(workers);return results;
}
function cloudWithTimeout(promise,label,timeoutMs=60000){
  let timeoutId;
  const timeout=new Promise((_,reject)=>{timeoutId=setTimeout(()=>reject(Error(`${label} TIMED OUT`)),timeoutMs)});
  return Promise.race([promise,timeout]).finally(()=>clearTimeout(timeoutId));
}
async function cloudSignedOptional(path){
  try{return await cloudWithTimeout(cloudSigned(path),'ASSET PREVIEW',15000)}
  catch(error){console.warn(`Cloud asset unavailable: ${path}`,error);return''}
}
...
async function cloudHydrate(kind,source){
  const data=structuredClone(source),fields=cloudAssetFields[kind]||[];
  for(const field of fields){const value=data[field];if(typeof value==='string'&&value.startsWith('storage://')){const path=value.slice(10);cloudAssetPaths.set(`${kind}:${data.id}:${field}`,path);data[`${field}StoragePath`]=path;data[field]=await cloudSignedOptional(path)}}
  if(kind==='character')for(const [index,v] of (data.variants||[]).entries())for(const asset of [{field:'gif',storageField:'gifStoragePath',key:'variant'},{field:'spawnImage',storageField:'spawnImageStoragePath',key:'variant-spawn'}]){const value=v[asset.field];if(typeof value==='string'&&value.startsWith('storage://')){const path=value.slice(10);cloudAssetPaths.set(`${kind}:${data.id}:${asset.key}:${v.id||index}`,path);v[asset.storageField]=path;v[asset.field]=await cloudSignedOptional(path)}}
  return data;
}
function cloudRecords(){return[{kind:'character',items:state.characters},{kind:'modifier',items:state.modifiers},{kind:'npc',items:state.npcs},{kind:'parasyte',items:parasytes},{kind:'achievement',items:achievements},{kind:'outreach',items:outreachRecords},{kind:'publisher',items:publisherRecords},{kind:'idea',items:ideaRecords},{kind:'idea_category',items:ideaCategoryRecords},{kind:'ongoing_task',items:ongoingTaskRecords},{kind:'task_option',items:taskOptionRecords}]}
...
async function cloudLoad(isRefresh=false){
  ...
  const characterMigration=migrateCharacterVariants(grouped.character),modifierWorkingMigration=grouped.modifier.some(modifier=>typeof modifier.working!=='boolean');state.characters=characterMigration.characters;state.modifiers=normalizeModifierWorkingRecords(grouped.modifier);ensureSpawnModifiersInState();state.npcs=grouped.npc;parasytes=grouped.parasyte;const recoveredMissingAchievements=!grouped.achievement.length&&achievements.length;if(grouped.achievement.length)achievements=mergeAchievementSeed(grouped.achievement);localStorage.setItem(ACHIEVEMENT_KEY,JSON.stringify(achievements));
  const {data:plan,error:planError}=await cloudClient.from('skinator_planner').select('data,updated_at').eq('id','main').maybeSingle();if(planError)throw planError;if(plan?.data){Object.keys(planner).forEach(k=>delete planner[k]);Object.assign(planner,plan.data)}const influencerAnalysisWasInitialized=!!planner.influencerAnalysisInitialized,businessInitialized=!!planner.businessInitialized,categoriesInitialized=!!planner.ideaCategoriesInitialized,tasksInitialized=!!planner.ongoingTasksInitialized,optionsInitialized=!!planner.taskOptionsInitialized,needsIdeaMigration=(planner.ideaSeedRevision||0)<IDEA_SEED_REVISION,needsPublisherMigration=(planner.publisherSeedRevision||0)<PUBLISHER_SEED_REVISION,recoveredMissingBusiness=(!grouped.outreach.length&&outreachRecords.length)||(!grouped.publisher.length&&publisherRecords.length)||(!grouped.idea.length&&ideaRecords.length)||(!grouped.idea_category.length&&ideaCategoryRecords.length)||(!grouped.ongoing_task.length&&ongoingTaskRecords.length)||(!grouped.task_option.length&&taskOptionRecords.length);if(grouped.outreach.length)outreachRecords=grouped.outreach;if(grouped.publisher.length)publisherRecords=grouped.publisher;if(grouped.idea.length)ideaRecords=grouped.idea;if(grouped.idea_category.length)ideaCategoryRecords=grouped.idea_category;if(grouped.ongoing_task.length)ongoingTaskRecords=grouped.ongoing_task;if(grouped.task_option.length)taskOptionRecords=grouped.task_option;if(needsIdeaMigration){mergeLatestIdeaSeed();planner.ideaSeedRevision=IDEA_SEED_REVISION}if(needsPublisherMigration){mergeLatestPublisherSeed();planner.publisherSeedRevision=PUBLISHER_SEED_REVISION}if(!businessInitialized)planner.businessInitialized=true;if(!categoriesInitialized)planner.ideaCategoriesInitialized=true;if(!tasksInitialized)planner.ongoingTasksInitialized=true;if(!optionsInitialized)planner.taskOptionsInitialized=true;localStorage.setItem(BUSINESS_KEY,JSON.stringify({outreach:outreachRecords,publishers:publisherRecords,ideas:ideaRecords,ideaCategories:ideaCategoryRecords,ongoingTasks:ongoingTaskRecords,taskOptions:taskOptionRecords,ideaSeedRevision:IDEA_SEED_REVISION,publisherSeedRevision:PUBLISHER_SEED_REVISION}));const influencerAnalysisSeeded=typeof ensureInfluencerAnalysisSeed==='function'&&ensureInfluencerAnalysisSeed({persist:false});cloudRememberSignature(rows,plan?.updated_at);
  const needsOutreachMigration=(planner.outreachSeedRevision||0)<OUTREACH_SEED_REVISION;if(needsOutreachMigration){mergeLatestOutreachSeed();planner.outreachSeedRevision=OUTREACH_SEED_REVISION;localStorage.setItem(BUSINESS_KEY,JSON.stringify({outreach:outreachRecords,publishers:publisherRecords,ideas:ideaRecords,ideaCategories:ideaCategoryRecords,ongoingTasks:ongoingTaskRecords,taskOptions:taskOptionRecords,ideaSeedRevision:IDEA_SEED_REVISION,publisherSeedRevision:PUBLISHER_SEED_REVISION,outreachSeedRevision:OUTREACH_SEED_REVISION}))}
  const taskSourcesChanged=typeof syncTaskSources==='function'&&syncTaskSources({persist:false}),achievementSeedMigration=!!window.skinatorAchievementSeedMigrated;window.skinatorAchievementSeedMigrated=false;cloudReady=true;cloudRenderActiveView();cloudSetReady(true);cloudSetState('ALL CHANGES SAVED',true);toast(characterMigration.migrated?`${characterMigration.migrated} VARIANTS CONVERTED TO CHARACTERS`:(isRefresh?'TEAM CHANGES LOADED':'SHARED TRACKER LOADED'));if(!businessInitialized||!categoriesInitialized||!tasksInitialized||!optionsInitialized||!influencerAnalysisWasInitialized||influencerAnalysisSeeded||recoveredMissingBusiness||recoveredMissingAchievements||achievementSeedMigration||needsIdeaMigration||needsPublisherMigration||needsOutreachMigration||taskSourcesChanged||characterMigration.changed||modifierWorkingMigration)window.skinatorCloudSave();
}
